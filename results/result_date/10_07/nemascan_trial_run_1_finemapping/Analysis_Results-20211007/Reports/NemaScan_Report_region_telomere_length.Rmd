...
finemap <- data.table::fread(glue::glue("{trait_name}.{QTL_chrom}.{QTL_start}.{QTL_end}.finemap_inbred.fastGWA")) %>%
  dplyr::mutate(CHR = as.factor(CHR),
                SNP = as.factor(SNP),
                POS = as.numeric(POS),
                AF1 = as.numeric(AF1),
                P = as.numeric(P))
ROI.geno.matrix <- data.table::fread(glue::glue("{trait_name}.{QTL_chrom}:{QTL_start}-{QTL_end}.ROI_Genotype_Matrix.tsv"))
ROI.LD <- data.table::fread(glue::glue("{trait_name}.{QTL_chrom}.{QTL_start}.{QTL_end}.LD.tsv"))
...
div_strain_list <- read.delim("div_isotype_list2.txt", header=FALSE, stringsAsFactors=FALSE) 
...
# genome bin table
all_QTL_bins <- read.delim("all_QTL_bins.bed", header=FALSE, stringsAsFactors=FALSE)
...
if (file.info("all_QTL_div.bed")$size!=0) {
    ...
    all_QTL_div <- read.delim("all_QTL_div.bed", header=FALSE, stringsAsFactors=FALSE)
...
haplotype_in_QTL_region <- read.delim("haplotype_in_QTL_region.txt", header=FALSE, stringsAsFactors=FALSE) %>% 
    dplyr::select(-V12)
